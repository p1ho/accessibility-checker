...
class ColorContrastChecker
{
  ...
  private static function _eval_DOM($dom_el,
                                    ...
                                    $in_mark) {
    // skip comments
    if (get_class($dom_el) === 'DOMComment') {return;}
    ...
    if ($style_str !== "") {
      $style_properties = self::_get_style_properties($style_str,
                                                      $tag_name,
                                                      $parent_bg_color,
                                                      $parent_font_color,
                                                      $parent_font_size,
                                                      $parent_is_bold); // need parent style in case of relative styles
   ...
   */
  private static function _get_style_properties($style_str,
                                                $tag_name,
                                                $parent_bg_color,
                                                $parent_font_color,
                                                $parent_font_size,
    ...
    if (isset($styles_raw[$property])) {
      ...
      } else if (self::_is_hex($value) || self::_is_color_name($value)) {

        $styles[$property] = $value;
        self::$parent_true_bg_color = $value;

      } else if ($value === "transparent" || $value === "initial") {
    ...
    if (isset($styles_raw['font-size'])) {
      ...
      $value = $styles_raw[$property];
      try {
        $styles[$property] = convert2pt($tag_name,
                                        $value,
                                        $parent_font_size,
    ...
    if (isset($styles_raw['font-weight'])) {
      ...
      $value = $styles_raw[$property];
      try {
        $styles['font_is_bold'] = self::_font_is_bold($tag_name,
                                                      $value,
                                                      $parent_is_bold);
      } catch (Exception $e) {
  ...
  private static function _color_function_to_array($s) {
    if (strpos($s, 'rgba(') === 0) {
      ...
      if (count($num_array) != 4) { $num_array[3] = 1; }
      return array(
        'r' => (float)$num_array[0],
        'g' => (float)$num_array[1],
        'b' => (float)$num_array[2],
  ...
  private static function _font_is_bold($tag_name, $value, $parent_is_bold) {
    if (in_array($value, array("bold","bolder"))) { return TRUE; }
    if (in_array($value, array("normal","lighter","unset"))) { return FALSE; }
    if ($value === "inherit") { return $parent_is_bold; }
    if ($value === "initial") {
